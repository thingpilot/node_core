...
void NodeFlow::start()
{
    ...
    starttime = time_now(); 
    starttime = fmod(starttime, DAYINSEC);
...
void NodeFlow::attachInterval(int (*user_def_function)(), uint32_t interval)
{
    ...
    u_conf.parameters.trigger_time = fmod(starttime+interval, DAYINSEC);
...
void NodeFlow::add(char* buffer, uint8_t b_size, uint8_t filename)
{
    ...
    status = DataManager::get_remaining_file_entries(filename, remaining_entries);
    if(status != NODEFLOW_OK)
    {
        debug("\r\nError.Line %d, Status: %d",__LINE__, status);
    }
    status = DataManager::get_total_written_file_entries(filename, written_entries);
    if(status != NODEFLOW_OK)
    {
        debug("\r\nError.Line %d, Status: %d",__LINE__, status);
    }
    ...
    {
        debug("Memory full\r\n");
        int bytes_to_be_removed=b_size*int(((written_entries+remaining_entries)/b_size)*0.25);
        debug("\r\nEntries to be removed %d\r\n",bytes_to_be_removed/b_size);
        is_overflow(filename, bytes_to_be_removed); 
    }
...
void NodeFlow::set_scheduler(int latency, uint32_t& next_timediff)
{
    ...
    {
        status = DataManager::read_file_entry(UserDefinedScheduler_n, i, u_conf.data, sizeof(u_conf.parameters));
        scheduled_times = fmod(u_conf.parameters.trigger_time, DAYINSEC);
    ...
    if(uck_flag.test(0)) //user defined true
    {
        int x = 0;
        for(int i = 0; i < entries; i++)
        {
            int time_now = fmod(latency_here,DAYINSEC);
           
            status = DataManager::read_file_entry(UserDefinedScheduler_n, i, u_conf.data, sizeof(u_conf.parameters));
            int next_time = u_conf.parameters.trigger_time;
            
            if(foo[x] == i && x < no_of_functions)
            {
                next_time = next_time+u_conf.parameters.interval_time;
                u_conf.parameters.trigger_time = fmod(next_time, DAYINSEC);
                if(x == 0)
                {
            ...
            if((u_conf.parameters.interval_time != 0) && (next_time < DAYINSEC)) //todo something wrong
            {
                while(time_now > next_time)
                {
                    next_time = next_time+u_conf.parameters.interval_time;
                }               
            }
